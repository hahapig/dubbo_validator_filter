...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements
	...
	public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor,
		...
		if(concurrencyLevel <= 0){
			throw new IllegalStateException("ConcurrencyLevel 必须为正数");
		}
		if(initialCapacity <= 0){
			throw new IllegalStateException("InitialCapacity 必须为非负数");
		}
		if(loadFactor <= 0){
			throw new IllegalStateException("loadFactor 必须为正数");
		}
